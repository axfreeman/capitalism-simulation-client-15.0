...
func (industry Industry) MoneyStock(timeStamp int) Industry_Stock {
	...
	stockList := *ClientLoggedInUsers[username].IndustryStocks(timeStamp)
...
func (class Class) MoneyStock(timeStamp int) Class_Stock {
	...
	stockList := *ClientLoggedInUsers[username].ClassStocks(timeStamp)
...
func (s Industry_Stock) OwnerName() string {
	username := s.UserName
	industryList := *ClientLoggedInUsers[username].Industries()
...
func (s Industry_Stock) Industry() *Industry {
	industryList := *ClientLoggedInUsers[s.UserName].Industries()
	for i := 0; i < len(industryList); i++ {
		ind := &industryList[i]
		if s.Industry_id == ind.Id {
			return ind
...
func (s Class_Stock) Class() *Class {
	classList := *ClientLoggedInUsers[s.UserName].Classes()
...
func (u User) Set_current_state(new_state string) {
	utils.TraceInfof(utils.Purple, "Enter set_current_state to %s for simulation with id %d", new_state, u.CurrentSimulationID)
	id := u.CurrentSimulationID
	sims := *u.SimulationsList()
	utils.TraceInfof(utils.BrightWhite, "resetting state to %s ", new_state)
	for i := 0; i < len(sims); i++ {
		s := &sims[i]
		if (*s).Id == id {
			(*s).State = new_state
		...
		utils.TraceErrorf("simulation with id %d not found", id)
	}
}
